...
const AUTH_ATTEMPT_THRESHOLD_PER_MINUTE = 10;
...
export const guestExpiryGuard = async (req, res, next) => {
  ...
  try {
    // Always fetch fresh from Firestore for guests — never trust cache for expiry
    // Cache is only safe for non-guests (registered users never expire)
    let userData = null;
    const cached = guardCache.getUser(req.uid);

    if (cached && cached.isGuest === false) {
      // Registered user — safe to use cache, they never expire
      userData = cached;
      showLogs("[guestExpiryGuard] registered user from cache, skipping expiry check");
      return next();
    }

    // Guest or unknown — always hit Firestore for accurate creationTime
    showLogs("[guestExpiryGuard] fetching fresh from Firestore for uid:", req.uid);
    const userSnap = await db.collection("users").doc(req.uid).get();

    if (!userSnap.exists) {
      showLogs("[guestExpiryGuard] user doc not found, allowing through");
      return next();
    }

    userData = userSnap.data();

    // Only cache if confirmed non-guest
    ...
    if (!isGuest) {
      showLogs("[guestExpiryGuard] not a guest, allowing through");
      return next();
    }

    // For guests: ONLY use creationTime from Firestore, never auth_time fallback
    // auth_time = token issue time, NOT account creation time — wrong for expiry
    const creationTime = userData.creationTime;

    if (!creationTime) {
      showLogs("[guestExpiryGuard] guest has no creationTime in Firestore — allowing through");
      return next();
    }
    ...
    showLogs(`[guestExpiryGuard] guest elapsed: ${elapsed}ms, limit: ${ONE_DAY}ms, creationTime: ${new Date(creationTime).toISOString()}`);
    ...
    showLogs("[guestExpiryGuard] guest still valid, allowing through");
    return next();
