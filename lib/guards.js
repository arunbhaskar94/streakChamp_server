...
export const guestExpiryGuard = async (req, res, next) => {
  ...
  try {
    // ✅ FIX: Never use cache for guest expiry check — always read fresh from Firestore
    let userData = null;
    const cachedData = guardCache.getUser(req.uid);

    if (cachedData && cachedData.isGuest !== true) {
      // Safe to use cache only for confirmed non-guest (registered) users
      userData = cachedData;
      showLogs("[guestExpiryGuard] using cached data — confirmed non-guest");
    } else {
      // Always fetch fresh from Firestore for guests or unknown users
      showLogs("[guestExpiryGuard] fetching fresh from Firestore...");
      ...
      guardCache.setUser(req.uid, userData); // cache it for non-guests only if needed
    }
